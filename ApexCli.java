...
import com.datatorrent.stram.api.Connect;
...
public class ApexCli
{
  ...
  //Added IOT MODE COMMANDS
  private final Map<String, CommandSpec> iotcommands = new TreeMap<>();
  ...
  {
    LOG.debug("Calling get Stram app");
  ...
  private static class CommandSpec
  {
    ...
    void verifyArguments(String[] args) throws CliException
    {
      ...
      if (args.length - 1 < minArgs || args.length - 1 > maxArgs) {
        throw new CliException(" Second Command parameter error");
      }
    }

    void printUsage(String cmd)
    {
      LOG.debug("Printing usage");
      System.err.print("*************** Printing usage ************* Usage: " + cmd);
      if (requiredArgs != null) {
  ...
  private static class OptionsCommandSpec extends CommandSpec
  {
    ...
    void verifyArguments(String[] args) throws CliException
    {
      try {
        ...
        throw new CliException("First Command parameter error");
      }
    }
  ...
  ApexCli()
  {
    ...
    //Added iot global commands
    globalCommands.put("connect-iot-app", new CommandSpec(new ConnectToIotApp(), new Arg[]{new Arg("app-id")},
        null,
        "Connect to an IOT app"));
    //
    ...
    // iot commands specification starts here
    iotcommands.put("show-iot-logical-plan", new CommandSpec(new GetIotLogicalPlan(),
        new Arg[]{new Arg("app-id")},
        null,
        "Get the logical plan of IOT app"));
    iotcommands.put("show-iot-app-details", new CommandSpec(new GetIotDetails(),
        new Arg[]{new Arg("app-id")},
        null,
        "Get the meta info of the IOT app"));
    iotcommands.put("list-iot-apps", new CommandSpec(new ListIotApp(),
        null,
        null,
        "Get the list of all the apps launched in IOT Dev Mode "));
    iotcommands.put("kill-iot-app", new CommandSpec(new KillIotApp(),
        new Arg[]{new Arg("app-id")},
        null,
        "Get the list of all the apps launched in IOT Dev Mode "));
    //
      ...
      try {
        ...
        if (commandLineInfo.args.length == 0) {
          ...
          if (fileName.endsWith(".json")) {
            ...
            AppPackage ap = null;
            try {
//              if (commandLineInfo.iotDevMode) {
//                LOG.debug("\n\nTHIS TIME DO NOT CLEAN\n\n");
//                AppPackage.clean = false;
//                String p = "/home/" + name  + "/Temporary";
//                ap = new AppPackage(new File(fileName), new File(p), true);
//              }
        ...
        if (appFactory != null) {
          if (!commandLineInfo.localMode && !commandLineInfo.iotDevMode) {
            ...
            ///////Added////////
          } else if (commandLineInfo.iotDevMode) {
            iot = true;
            LOG.debug("\n\n Added local dev mode in commandline info \n\n");
            submitApp.runIotDev(appFactory);
              //////////////////////
          } else {
...
//  }
  ...
  private class ShowLogicalPlanCommand implements Command
  {
    ...
    public void execute(String[] args, ConsoleReader reader) throws Exception
    {
      ...
      if (commandLineInfo.args.length > 0) {
        ...
        LOG.debug("PATH_LOGICAL_PLAN " + StramWebServices.PATH_LOGICAL_PLAN);
  ...
  private class ListIotApp implements Command
  {
    ...
    public void execute(String[] args, ConsoleReader reader) throws Exception
    {
      String name = UserGroupInformation.getLoginUser().getUserName();
      try {
        FileReader fr = new FileReader(new File("/home/" + name + "/MetaFile"));
        ...
        if (flag == false) {
          System.out.println("No apps have been launched in this mode");
        }
        fr.close();
        br.close();
      } catch (FileNotFoundException fn) {
        System.out.println("No iot apps to show");
      }

    }
  }

  private class GetIotDetails implements Command
  {
    @Override
    public void execute(String[] args, ConsoleReader reader) throws Exception
    {
      ...
      String name = UserGroupInformation.getLoginUser().getUserName();
      try {
        FileReader fr = new FileReader(new File("/home/" + name + "/IotApps/AppMeta" + appid));
        BufferedReader br = new BufferedReader(fr);
        String line;
        ...
        System.out.println("No app found with the specified app ID ");
      }
...
//      printJson(json);
  ...
  private class GetIotLogicalPlan implements Command
  {
    ...
    public void execute(String[] args, ConsoleReader reader) throws Exception
    {
      ...
      try {
        ...
        JSONObject json = new JSONObject(lp);
        printJson(json);
      } catch (FileNotFoundException fn) {
        System.out.print("No app found with the specified app ID");
      }
    }
  }
  ...
  private class KillIotApp implements Command
  {
    ...
    public void execute(String[] args, ConsoleReader reader) throws Exception
    {
      ...
      try {
        File f = new File("/home/" + name + "/IotApps/AppMeta" + appid);
        FileReader fr = new FileReader(f);
        BufferedReader br = new BufferedReader(fr);
        String s;
        String pid;
        while ((s = br.readLine()) != null) {
          if (s.contains("Process ID: ")) {
            pid = s.substring(12, s.length() - 2);
            System.out.println("killing " + pid);
            pid = pid.trim();
            kill(Long.parseLong(pid));
            String command = "kill -1 " + pid;
//            Process p;
//            try {
...
//            }
        ...
        BufferedReader reader1 = new BufferedReader(new FileReader(new File("/home/" + name + "/MetaFile")));
        s = "";
        ...
        FileWriter fw = new FileWriter(new File("/home/" + name + "/MetaFile"));
    ...
    public  void kill(long pid) throws IOException, InterruptedException
    {
      String line = "kill " + pid;
      ...
      try {
        ...
        org.apache.commons.exec.CommandLine commandLine = org.apache.commons.exec.CommandLine.parse("bash killProcess.bash");
        try {
...
//////////////////////////////////////////////////////////////////////////////
  ...
  public static class LaunchCommandLineOptions
  {
    ...
    /////////Added//////////
    final Option iotdev = add(new Option("iotdev", "Run application in local development mode as an asynchronous process" ));
    /////////Added/////////
  ...
  static LaunchCommandLineInfo getLaunchCommandLineInfo(String[] args) throws ParseException
  {
    ...
    /////////// Added ///////////
    result.iotDevMode = line.hasOption(LAUNCH_OPTIONS.iotdev.getOpt());
    /////////////////////////////
